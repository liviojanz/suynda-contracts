 ...
 */
export const PADRON_PARTIES_PATH = "/padron/v1/parties" as const;

/** El único query param del GET de búsqueda (`parties.ts:316-323`). */
export const PADRON_PARTIES_IDENTIFIER_QUERY = "identifier" as const;

/** `GET /padron/v1/parties/:id` (`parties.ts:339`). */
export function padronPartyPath(partyId: string): string {
  return `${PADRON_PARTIES_PATH}/${encodeURIComponent(partyId)}`;
}

/** `GET /padron/v1/parties/:id/roles` (`parties.ts:349`). */
export function padronPartyRolesPath(partyId: string): string {
...
export function parsePartyIdentifier(raw: string): ParsePartyIdentifierResult {
  if (typeof raw !== "string") return { ok: false, reason: "forma" };
  const first = raw.indexOf(PARTY_IDENTIFIER_SEPARATOR);
  if (first === -1) return { ok: false, reason: "forma" };
  const second = raw.indexOf(PARTY_IDENTIFIER_SEPARATOR, first + 1);
  if (second === -1) return { ok: false, reason: "forma" };
  const tipo = raw.slice(0, first);
  const pais = raw.slice(first + 1, second);
  const valor = raw.slice(second + 1);
  if (!isIdentifierType(tipo)) return { ok: false, reason: "tipo" };
  if (valor.length === 0) return { ok: false, reason: "valor" };
